...
.las文件是一个二进制文件，需要通过下表的结构访问。

在头文件里，

![image-20201028001138495](image/LAS/image-20201028001138495.png)

![image-20201028002236287](image/LAS/image-20201028002236287.png)

​								

```
offset to point data :可以直接从文件开头跳到点的位置。
point data record length :记录每个点包含的字节（26,57，63等）
Xcoordinate = (Xrecord * Xscale) + Xoffset 
Ycoordinate = (Yrecord * Yscale) + Yoffset  
Zcoordinate = (Zrecord * Zscale) + Zoffset 
```

​								 ![image-20201028003059677](image/LAS/image-20201028003059677.png)

```
x,y,z是整数，需要乘比例系数加偏移量
```

[LAS文件结构](https://wenku.baidu.com/view/4a8d7b3083c4bb4cf7ecd122.html#) 
...
> 1. lasinfo    查看数据
> 2. las2las    移除不想干的VLR  更改坐标系比例
> 3. 用lassort   一个最大化压缩，一个提高空间相干性

> 1. lastile  将数据平铺起来   把点分类设置为0
> 2. las2las  提取强度值小于1000的点，设置分类值为7（噪声）
> 3. 用lasnoise   忽略分类值为7的点，将剩下的点继续噪声提取，方法是在一定大小方格内寻找孤立点，这些点标记为9
> 4. 